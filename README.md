...
A standalone React chat app backed by one Node server, Gemini, PI's durable AgentHarness, and SQLite. Supports streamed answers, saved conversations, Stop, and interrupted chat recovery. Attach a separately running Studio to move, rotate, or remove placed objects through the decorator agent. General chat works without a Studio.
...
The companion Studio adapter imports the shared contract and connects to `/ws` using the bootstrap server ID. It registers its design/tab, subscribes to its private mailbox, then calls `ready`. It answers fresh-context requests, executes each command once against the current room, and reports its save result. See [the contract and coordinate mapping](docs/Studio-Contract.md).

Build consumable packages for the separate Studio checkout with:

```sh
pnpm pack:studio --out /tmp/livi-studio-direct-contracts
```

Install all tarballs listed by `/tmp/livi-studio-direct-contracts/handoff.json` together in the Studio checkout, for example `npm install --ignore-scripts --omit=optional /tmp/livi-studio-direct-contracts/*.tgz`. External consumers import `@livi/studio-contracts`; workspace consumers use `@livi/decorator-agent/contracts`. The handoff verifies an isolated consumer's types, browser bundle, and runtime imports. Do not copy contract definitions or depend on this repository's `workspace:*` links.

In chat, select a conversation, choose a connected Studio, and click **Attach design**. The panel shows the attached design, selected placed objects, and connection availability. Changing conversations and changing a design are separate controls. An earlier missing action result does not lock the room.
...
Only move, yaw rotation, and remove are supported. Moves use metres: +X right, +Y back, +Z up, from the floor's front-left. Rotation uses radians around +Z. “Move it back” and “undo that rotation” use already completed tool history with saved before/after transforms. An intervening change to that object's transform prevents reversal. A rejected reversal blocks further room edits in that response; clarify with a new message. Removal cannot be reversed; adding, replacing, restoring, materials, and layout generation are outside this scope.

If an ordinary action is rejected because its revision is stale, the agent calls `get_room_context`, inspects the latest inventory, and recalculates within the same request, with at most two retries. Refresh feeds the next model generation; precomputed actions in the same tool batch retain their original revision. Unknown outcomes and failed reversals cannot be retried through refresh. This reads the attached Studio’s current state, not backend-only changes from other tabs.

Studio owns the current room and saving. The agent reads Studio context, sends each tool action once, and reports the supplied saved or error result. If the reply is lost, the agent reports that no result was received. It does not query status, retry the same edit in that response, or block the next explicit user request. Reconnect refreshes the attachment and current context without resending commands. Old unresolved records do not lock designs after restart.

Stop prevents unsent work and ends the model response. An action already sent may still save in Studio; cancellation does not undo it. Ordinary conversation persistence and interrupted chat recovery remain available, but interrupted room actions are never automatically replayed.

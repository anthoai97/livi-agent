...
import { Server, SessionNotFoundError, type ServerHost } from '@earendil-works/pi-server';
import { createNodeSqliteFactory, SqliteSessionRepo } from '@earendil-works/pi-session-backend-sqlite-node';
import { DecoratorSession, createServerServices } from '@livi/decorator-agent';
...
export async function startLiviServer(options: LiviServerOptions = {}) {
  ...
  const clientDirectory = resolve(options.clientDirectory ?? fileURLToPath(new URL('../../client/dist', import.meta.url)));
  ...
  const repo = new SqliteSessionRepo({ directory: resolve(dataDirectory, 'sessions'), databaseFactory: createNodeSqliteFactory() });
  const reportError = options.onError ?? ((error: Error) => console.error(error));
  const services = await createServerServices({
    list: async () => (await repo.list(undefined, context)).map(({ id, createdAt }) => ({ serverId, sessionId: id, createdAt })),
    create: async () => {
  ...
  const host: ServerHost<Awaited<ReturnType<SqliteSessionRepo["list"]>>[number]> = {
    ...
    async openSession(metadata) {
      ...
      try {
        return await DecoratorSession.create({ session, models: options.models, modelId: options.modelId, apiKey: options.apiKey, onError: reportError });
      } catch (error) {
        await session.close(context);
        throw error;
      }
  ...
  const protocol = new Server(host, { serverId, listeners: [], onError: reportError, onConnectionCountChanged: (count) => { connectionCount = count; } });
  ...
  const http = createServer((request, response) => {
    void (async () => {
      ...
      try {
        ...
        response.writeHead(404, { 'content-type': 'text/plain' }).end('Frontend unavailable. Run pnpm build or pnpm dev.');
      }
    })().catch((error: unknown) => {
  ...
  http.on('upgrade', (request, socket, head) => {
    ...
    sockets.handleUpgrade(request, socket, head, (websocket) => {
      const handler = protocol.accept({
        get closed() { return websocket.readyState !== WebSocket.OPEN; },
        send(chunk) {
          return new Promise<void>((resolveSend, reject) => websocket.send(chunk, { binary: true }, (error) => error ? reject(error) : resolveSend()));
        },
      ...
      websocket.on('message', (data, binary) => {
        if (!binary) { websocket.close(1003, 'Binary PI frames required'); return; }
        handler.onData(Array.isArray(data) ? Buffer.concat(data) : data instanceof ArrayBuffer ? new Uint8Array(data) : data);
      });
  ...
  const close = () => closing ??= (async () => {
    ...
    for (const cleanup of [
      ...
      () => new Promise<void>((done, reject) => http.close((error) => error && !('code' in error && error.code === 'ERR_SERVER_NOT_RUNNING') ? reject(error) : done())),
    ]) {
  ...
  try {
    await new Promise<void>((done, reject) => {
      ...
      http.listen(options.port ?? 3001, options.host ?? '127.0.0.1', () => { http.off('error', reject); done(); });
    });
  } catch (error) {
  ...
  return { serverId, port: address.port, close, get connectionCount() { return connectionCount; } };
}

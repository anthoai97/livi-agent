import { createRemoteServiceEndpoint, RemoteServiceProvider, replicatedState, type Context } from "@earendil-works/chord";
import { BACKGROUND_CONTEXT } from "@earendil-works/chord/context";
import { AgentHarness, HarnessClosed, DEFAULT_COMPACTION_SETTINGS, type AgentLane, type Session } from "@earendil-works/pi-agent-core";
...
export class DecoratorSession implements RoutedSessionHandle {
    ...
    this.controller = {
      prompt: async (request, context) => {
        ...
        if (!result.ok) return {
          ...
          error: { code: result.error._tag === "LaneBusy" ? "lane_busy" : result.error._tag === "InvalidMessage" ? "invalid_message" : "operation_failed", message: result.error.message },
        };
  ...
  static async create(options: DecoratorSessionOptions, context: Context = BACKGROUND_CONTEXT): Promise<DecoratorSession> {
    let registry = options.models;
    if (registry === undefined) {
      const models = createModels(options.apiKey === undefined ? undefined : {
        authContext: {
    ...
    const { harness, open } = await AgentHarness.create({
      ...
      systemPrompt: "You are Livi, a helpful assistant for general questions and interior decoration advice. Answer in the user’s language, using the information they supply.",
      compaction: { ...DEFAULT_COMPACTION_SETTINGS, enabled: false },
    }, context);
    let runtime: DecoratorSession | undefined;
    try {
      if (open.some((operation) => operation.lane !== "main")) throw new Error("Decorator sessions support only the main lane");
  ...
  private startDrive(operationId: string): void {
    ...
    const pending = this.lane.drive({ operationId, waitForRetry: true, pollDeferred: true }, BACKGROUND_CONTEXT).then((result) => {
      if (!result.ok) throw result.error;
    }).catch((error: unknown) => {
      if (!(this.closing && error instanceof HarnessClosed)) this.onError(error instanceof Error ? error : new Error(String(error)));
  ...
  attachClient(): RoutedSessionAttachment {
    ...
    const provider = new RemoteServiceProvider([{ service: AgentController, mode: "singleton" }, { service: Transcript, mode: "singleton" }]);

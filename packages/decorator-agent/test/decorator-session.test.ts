...
it("returns durable admission while generation runs and survives caller cancellation", async () => {
  ...
  expect(await runtime.controller.prompt({ message: "Duplicate" }, context)).toMatchObject({ accepted: false, error: { code: "lane_busy" } });
  release.resolve();
...
it("aborts a streaming operation and permits the next prompt", async () => {
  const { runtime, faux } = await fixture();
  const started = Promise.withResolvers<void>();
  faux.setResponses([async (_input, options) => {
    started.resolve();
...
it("rejects unsolicited tool calls without an execution environment", async () => {
  ...
  faux.setResponses([
    ...
    (input) => {
      expect(input.tools ?? []).toEqual([]);
      expect(input.messages).toContainEqual(expect.objectContaining({

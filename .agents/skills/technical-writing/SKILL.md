...
The goal is writing a tired engineer understands on the first read. Four layers get you there, one question each: what kind of document is this, how do sentences address the reader, how much does each sentence carry, and can any sentence be read two ways. Apply the layers relevant to the task.
...
Don't invent jargon. Use the words a developer would say out loud: "move", "delete", "a budget that only decreases", not "evacuate", "ratchet", or "endgame". A named pattern is fine when the doc says what it means the first time. Propose changes to unslop only when the user requests skill maintenance.
...
Apply document-structure rules when creating or restructuring documentation. Preserve existing structure for scoped edits.
...
Apply to any prose this skill covers. Item 1 applies only when creating or restructuring documentation:

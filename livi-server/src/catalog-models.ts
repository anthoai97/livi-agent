...
export function createCatalogModels(options: { apiKey?: string; modelId?: string }): CatalogModels {
		...
		async embedQuery(query, signal) {
			...
			return catalogDeadline(async (active) => {
				try {
					...
					throw new CatalogError("model_failed", "Catalog embedding model failed", { stage: "embed" });
				}
			}, signal);
		},
	};
